...
that means a variable that keeps track of average has no chance of overflowing.
IF WE recalculate the AVERAGE after every iteration until we reach the sentinel

tldr: use a running average strategy to solve overflow and underflow of sum, discarding
the use of a sum variable 

to solve overflow on counter, we can set it to be a double. double's do not overflow
like integers, and instead result in a loss of precision. this loss of precision is
extremely minute, to the point if negligiblity. 
